...
在设计前馈ANN时，我们需要先考虑一些因素。第一是使用什么激活功能。我们之前提到激活函数作为节点输出的决策者。激活函数决定的类型将帮助我们决定使用哪一个。在我们的例子中，我们将设计一个ANN，为每个数字（0-9）输出一个介于0和1之间的值。值越接近1意味着ANN预测这是绘制的数字，值越接近0意味着它被预测不是绘制的数字。因此，我们想要一个激活函数，其输出接近0或接近1.我们还需要一个可微分的函数，因为我们将需要导数用于反向传播计算。在这种情况下常用的函数是S形，因为它满足这两个约束。 StatSoft提供了一个很好的常用激活函数及其属性列表。[nice
